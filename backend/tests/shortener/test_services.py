...
async def test_create_db_url(async_session: AsyncSession) -> None:
    ...
    db_url = await create_url(session=async_session, url=url)
...
async def test_get_db_url_by_key(async_session: AsyncSession) -> None:
    ...
    Args:
        async_session (AsyncSession): The database session.
    """
    db_url = await create_url(session=async_session, url=SUrlBase(target_url="https://example.com"))
    db_url_from_db = await get_active_long_url_by_key(session=async_session, url_key=db_url.key)
